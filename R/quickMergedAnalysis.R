#' Perform a merged scRNA-seq analysis
#'
...
#'
#' @export 
quickMergedAnalysis <- function(x, 
    batch,
    qc.subsets,
    qc.nmads=3,
    trend.span=0.3,
    ...
    results$qc.filters <- qc.filters <- perCellQCFilters.chan(qc.metrics$sums, qc.metrics$detected, qc.metrics$subsets, nmads=qc.nmads, batch=batch)
    results$qc.discard <- qc.filters$filters$overall
    x <- filterCells.chan(x, results$qc.discard)
    batch <- batch[!results$qc.discard]

    sf <- qc.metrics$sums[!qc.filters$filters$overall]
    sf <- sf/mean(sf)
    results$size.factors <- sf
    x <- logNormCounts.chan(x, sf, batch=batch)

    results$variances <- modelGeneVar.chan(x, span = trend.span, num.threads=num.threads, batch=batch)
    keep <- rank(-results$variances$statistics$residuals, ties.method="first") <= hvg.num
    results$variances$keep <- keep

    correct.method <- match.arg(correct.method)
    if (correct.method == "regress") {
        results$pca <- runPCA.chan(x, num.comp=pca.num, subset=keep, num.threads=num.threads, batch=batch, batch.method="block")
        pcs <- results$pca$components 
        results$pca$components <- t(pcs)
    } else {
        results$pca <- runPCA.chan(x, num.comp=pca.num, subset=keep, num.threads=num.threads, batch=batch, batch.method="weight")
        results$mnn <- mnnCorrect.chan(results$pca$components, batch=batch)
        pcs <- results$mnn$corrected
        results$mnn$corrected <- t(pcs)
    }
    ...
    for (s in markers.pref) {
        ...
        if (!is.null(clusters)) {
            results$markers <- scoreMarkers.chan(x, clusters, batch=batch)
            break
        }
    }
